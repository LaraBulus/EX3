...
public class MyGameGUI extends JFrame implements ActionListener, MouseListener{
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(str.equals("Manual")) {
			...
			this.mg = new ManualGame();

			mg.setNodesGuiLocations(this.graph);
			mg.setFruitsGuiLocations(this.fruits);
			mg.setRobotGuiLocations(this.robots);
			...
			Runnable move = new Runnable() {
				...
				public void run() {
					...
					g2d.drawString("Game Over: " + results, 800, 550);
		...
		//Automatic Game
			...
			ag.buildRobotsPath(this.graph, this.robots, this.fruits);
			game.startGame();
			threadListeningToKml();
	...
	/**
	 * Function that choose the next node for the robot (Auto mode)
	 * 
	 * @param id - robot id
	 * @param p - robot location
	 */
	private int nextNode(int id , Point3D p) {
		int dest = -1;
		for(GraphRobot r : robots) {
			if(r.getId() == id && r.getPath().size() > 0) {
				dest = r.getPath().get(0).getKey();
				r.getPath().remove(0);
				r.setSrc(dest);
				r.setPos(p);
				if(r.getPath().size() == 0) {
	...
	public void mousePressed(MouseEvent m) {
		...
		if (clicked == 2) {
			robotID = -1;
			nodeID = -1;
			GraphRobot r = mg.findRobotByLocation(p, this.robots);
			if (r != null) 
				robotID = r.getId();
		}
		else if(clicked == 4) {
			...
			node_data n = mg.findNodeByLocation(p, r.getSrc(), this.graph);
			if (n != null) 
				nodeID = n.getKey();
			clicked = 0;
		}
